...
\begin{defi}{Ordre bien fondé. $\star$}{}
    On dit qu'un ordre $\leq$ sur un ensemble $E$ est \bf{bien fondé} si il vérifie l'une des deux conditions suivantes :\\
    \hspace*{2em}$1.$ Tout sous-ensemble non-vide de $E$ admet un élément minimal.\\
    \hspace*{2em}$2.$ Toute suite infinie décroissante de $E$ est stationnaire.\\
    Ces conditions sont équivalentes.
    \tcblower
    Montrons l'équivalence des conditions.\\
    ...
    Soit $F=\{u_n ~ | ~ n \in \mathbb{N}\}\subset E$. Alors $\exists k \in \mathbb{N} ~ | ~ u_k = \min(F)$, or $(u_n)$ est décroissante donc $\forall n \geq k, ~ u_n = u_k$.\\
    On a bien montré que cette suite est stationnaire.\\[0.2cm]
    \fbox{$\la$} Supposons que toute suite infinie décroissante de $E$ est stationnaire. Soit $F\subset E$ tel que $F \neq \0$.\\
    On définit $(u_n)$ : $u_0 \in F$ et pour $n\in\N$, $u_{n+1}$ est le prédécesseur immédiat de $u_n$ par $\leq$ s'il existe, sinon $u_n$.\\
...
\begin{defi}{}{}
    Soit une famille $(E_i,\leq_i)_{i\in I}$ d'ensembles ordonnés. L'ordre produit sur $\prod_{i\in I}E_i$ est donné par:
    \begin{equation*}
...
\begin{corr}{Ordre lexicographique.}{}
    Soit $E$ un ensemble ordonné par $\leq$. L'ordre lexicographique sur $E^n$ est :
    \begin{equation*}
...
\begin{defi}{Ensemble défini inductivement. $\star$}{ind}
    Soit $E$ un ensemble non vide. Une définition de $X \subseteq E$ consiste à se donner :\\
    $\hra$ Un ensemble $B\subseteq E$ non vide d'assertions.\\
    $\hra$ Un ensemble $R$ de règles : $\forall r_i \in R, ~ r_i : E^{n_i} \to E$ avec $n_i$ l'arité de $r_i$.
\end{defi}
...
\begin{thm}{Point fixe. $\star$}{}
    ...
    \end{equation*}
    Puisque $\forall Y\in \cursive{F}, ~ B\subset Y$, on en déduit que $B \subset X$. On a donc vérifié $(B)$. Soit $r_i\in R$ et $(x_1, ..., x_{n_i})\in X^{n_i}$.\\
...
\begin{thm}{Induction structurelle. $\star$}{}
    Soit $X\subseteq E$ défini inductivement (cf \ref{defi:ind}) et $\m{P}$ un prédicat sur $E$. On suppose :\\
    \hspace*{2em}$(B)$\hspace*{0.025cm}: $\m{P}(x)$ est vraie pour tout $x\in B$.\\
...
\begin{corr}{Induction bien fondée.}{}
    ...
    \tcblower
    Les assertions sont les minimaux, les règles sont des fonctions permettant de passer au successeur immédiat.
\end{corr}

\subsection{Correction des fonctions récursives.}

\begin{meth}{Correction des fonctions récursives (par induction).}{}
