...
\begin{defi}{Algorithmes gloutons.}{}
    ...
    \bf{Contraintes.} La solution n'est pas toujours optimale.
\end{defi}
...
\begin{ex}{Placement d'activités.}{}
    ...
    On peut alors construire $(c_1,...,c_n,c_{n+1})$, car $d_{n+1}$ est pla\cc{c}able : contradiction.\\
    \bf{Hérédité.} Pour $k<n$, il ne peut pas exister une suite de cours pla\cc{c}able de longueur $n+1$ ayant un préfixe commun de longueur au moins $k+1$ avec $(c_1,...,c_n)$.\\
    Supposons qu'il existe une suite de cours pla\cc{c}ables $(c_1,...,c_k,d_{k+1},...,d_{n+1})$, $c_{k+1}$ est pla\cc{c}able entre $c_k$ et $d_{k+1}$, ce qui est absurde.
\end{ex}
