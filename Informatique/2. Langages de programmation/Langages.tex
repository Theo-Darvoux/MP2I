...
\begin{defi}{Entiers.}{}
    ...
    $\hra$ Des types dont la taille est fixée par la norme : \texttt{int8\_t}, \texttt{uint\_8t}, \texttt{int32\_t}, ...\n
...
\begin{defi}{Variable.}{}
    ...
    $\hra$ La façon dont les données sont interprétées.\\
...
\begin{defi}{Règles.}{}
    ...
    $\hra$ Éviter les underscore en premier caractère.
\end{defi}
...
\begin{defi}{Allocation.}{}
    ...
    On accède au contenu de la $i^{\nt{ème}}$ case d'un pointeur \texttt{p} en écrivant \texttt{p[i]}.
\end{defi}
...
\begin{defi}{La pile.}{}
    ...
    $\hra$ L'adresse de l'instruction à exéctuer après le retour.\n
...
\begin{defi}{Types de base.}{}
    ...
    $\hra$ \ocaml{int} : entiers \texttt{(+, -, *, /, mod)} codé sur 63 bits.\\
    $\hra$ \ocaml{float} : flottants \texttt{(+., -., *., /.)} en norme IEE.\\
    $\hra$ \ocaml{bool} : booléens \texttt{(\&\&, ||, not)}.\\
    $\hra$ \ocaml{char} : caractères codé sur 8 bits en ASCII.\\
...
\begin{defi}{Curryfication.}{}
    ...
    La notation \texttt{fun x y $\to$ e} est un sucre syntaxique pour \ocaml{fun x $\to$ (fun y $\to$ e)}.\\
...
\begin{defi}{Fonctionnement du filtrage.}{}
    ...
    $\hra$ Si \texttt{v} s'identifie avec le premier motif possible.\\
