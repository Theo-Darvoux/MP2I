...
\begin{defi}{Liste chaînée.}{}
    ...
    $\hra$ Soit c'est un noeud, appelé \bf{cellule}, qui contient une valeur et une référence à une autre liste chaînée.
\end{defi}

\vspace*{-0.7cm}

\begin{defi}{Pile.}{}
    Une \bf{pile} est une structure de données qui permet de stocker des éléments et de les retirer dans l'ordre inverse de leur insertion. On peut :\\
    $\hra$ \bf{Empiler} un élément.\\
    $\hra$ \bf{Dépiler} un élément.\\
    $\hra$ \bf{Tester} si la pile est vide.
\end{defi}

\vspace*{-0.7cm}

\begin{defi}{File.}{}
    Une \bf{file} est une structure de données qui permet de stocker des éléments et de les retirer dans l'ordre de leur insertion. On peut :\\
    $\hra$ \bf{Enfiler} un élément.\\
    $\hra$ \bf{Défiler} un élément.\\
    $\hra$ \bf{Tester} si la file est vide.
\end{defi}
...
\begin{defi}{Arbre binaire.}{}
    ...
    $\hra$ Soit c'est un arbre vide.\\
    $\hra$ Soit il est constitué d'un noeud et de deux arbres binaires disjoints, appelés \bf{fils gauche} et \bf{fils droit}.
\end{defi}
...
\begin{defi}{Parcours en profondeur. $\star$}{}
    ...
    $\hra$ \bf{Préfixe} : on visite le noeud, puis le fils gauche, puis le fils droit.\\
    $\hra$ \bf{Infixe} : on visite le fils gauche, puis le noeud, puis le fils droit.\\
    $\hra$ \bf{Postfixe} : on visite le fils gauche, puis le fils droit, puis le noeud.
\end{defi}
...
\begin{defi}{File de priorité.}{}
    Une \bf{file de priorité} est une structure de données qui permet de stocker des éléments d'un ensemble totalement ordonné. On peut :\\
    $\hra$ Insérer un nouvel élément.\\
    $\hra$ Extraire l'élément le plus grand.\\
    $\hra$ Modifier la valeur d'un élément.\\
    $\hra$ Tester si la file est vide.
\end{defi}
...
\begin{defi}{Percoler vers le bas. $\star$}{}
    \hspace*{0.2cm}\begin{algorithm}[H]
        \LinesNumbered
...
\begin{defi}{Arbre binaire de recherche.}{}
    Un \bf{arbre binaire de recherche (ABR)} a des noeuds étiquettés dans un ensemble totalement ordonné et tel que l'étiquette de chaque noeud interne est supérieure aux étiquettes de son sous-arbre gauche, et inférieure aux étiquettes de son sous-arbre droit.\\
...
\begin{prop}{Parcours infixe. $\star$}{}
    Soit $\cursive{A}$ un ABR et $\P_h$ : <<Le parcours infixe de $\cursive{A}$ de hauteur $h$ donne une liste triée>>.
    \tcblower
    \bf{Initialisation.} Pour $h=0$ c'est trivial car l'arbre est réduit à sa racine.\\
    \bf{Hérédité.} Soit $h\in\N$ tel que $\forall k < h, ~ \P_k$. Montrons $\P_h$.\\
    ...
    Le parcours infixe parcourt d'abord $\cursive{A}_g$, puis $r$, puis $\cursive{A}_d$.\\
    Tout élément de $\cursive{A}_g$ est inférieur à $r$ et tout élément de $\cursive{A}_d$ est supérieur à $r$ par propriété des ABR.\\
...
\begin{defi}{Arbre rouge-noir. $\star$}{}
    ...
    $\hra$ Chaque feuille est noire.\\
    $\hra$ La racine est noire.\\
    $\hra$ Les fils d'un noeud rouge sont noirs.\\
    $\hra$ Tout chemin de la racine à une feuille contient le même nombre de noeuds noirs.
\end{defi}
...
\begin{thm}{Complexité de tris par comparaisons. $\star$}{cmp}
    ...
    On note $h$ la hauteur de cet arbre, alors $2^h \geq n!$ donc $h\geq\log(n!)$, puis :
    \begin{equation*}
        h\geq\log(n!)=\sum_{k=1}^n\log(k)\geq\sum_{k=n/2}^n\log(k)\geq\frac{n}{2}\log\left(\frac{n}{2}\right).
    \end{equation*}
    Conclusion : $h=\Omega(n\log n)$.
\end{thm}
...
\begin{prop}{}{}
    Le tri rapide termine et est correct.
    \tcblower
    \textbf{Terminaison.}\\
...
\begin{prop}{}{}
    Le tri rapide a une complexité en $O(n^2)$ dans le pire des cas, et en $O(n\log n)$ en moyenne.\\
    \textbf{Complexité.}\\
    ...
    Si les données de départ sont dans un ordre aléatoire uniforme, alors c'est le cas de tout sous-tableau de l'entrée.\\
...
\begin{defi}{}{}
    ...
    On appelle \bf{clé} un élément de l'ensemble de départ et \bf{valeur} son élément associé. Interface :\\
    $\hra$ \bf{Insertion} (clé, valeur).\\
    $\hra$ \bf{Recherche} de clé.\\
    $\hra$ \bf{Suppression} de clé.\n
    Contraintes:\\
    $\hra$ Les clés sont toutes de mêmes types.\\
    $\hra$ Les valeurs sont toutes de mêmes types.\\
    $\hra$ Les clés sont uniques.\\
...
\begin{defi}{}{}
    Principe : S'il n'y a pas de place, on en cherche ailleurs. Comment choisir le <<ailleurs>> ?\\
