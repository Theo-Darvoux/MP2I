...
\begin{defi}{Sous-graphes}{}
    Soit un graphe $G=(S,A)$. Le graphe $G'=(S',A')$ est un \textbf{sous-graphe} de $G$ ssi $S'\subset S$ et $A'\subset A$.\\
...
\begin{prop}{}{}
    Soit $G=(S,A)$ un graphe $G$ possède $2^{|S|}$ sous-graphes induits.
    \tcblower
    Pour chaque sommet, on a le choix de le mettre ou non, d'où $2^{|S|}$ possibilités.
\end{prop}
...
\begin{prop}{Équivalences $\star$}{}
    ...
    \fbox{$3\Rightarrow4$} Supposons $G$ connexe minimal. Soient $s,t\in S$ distincts. Il existe un chemin élémentaire entre $s$ et $t$ car $G$ est connexe. Montrons qu'il est unique.\\
    ...
    \fbox{$4\Rightarrow5$} Montrons que $G$ est acylique par l'absurde. Soit un cycle de $G$ et $s,t$ deux sommets distincts de ce cycle.
...
\begin{prop}{Tri topologique de Kahn. $\star$}{}
    ...
    \hspace*{0.28cm}\begin{algorithm}[H]
        \LinesNumbered
...
\begin{prop}{}{}
    Le tri topologique de Kahn a une correction totale.
    \tcblower
    Montrons que les propriétés suivantes forment un invariant de la boucle ligne 8 :
    \begin{itemize}
...
\begin{defi}{Matrice d'adjacence $\star$}{}
    ...
    \textbf{Remarque:} Dans le cas non-orienté, la matrice est symétrique et sa diagonale ne contient que des 0.  
\end{defi}
...
\begin{prop}{}{}
    ...
    \tcblower
    On note $C_n(s,t)$ le nombre de chemins de longueur $n$ de $s$ vers $t$. Par récurrence sur $n$.\\
...
\begin{prop}{}{}
    ...
    Le variant de boucle est $n-k$. Ces deux variants assurent la terminaison car les autres instructions se terminent.\\
    \textbf{Correction:} Par récurrence sur $len$. \textbf{Cas de base:} Si $len=0$, correct par la ligne 15.\\
...
\begin{prop}{}{}
    ...
    Alors $T(l)\leq n^lT(0) + 2\frac{1-n^{l+1}}{1-n}=O(n^l)$. Soit $G=(S,A)$ où $S=\{s_1,...,s_n\}$ et $A=\{s_i\to s_j ~ | ~ i,j\in\lb0,n-2\rb\}$.\\
    L'appel à chemin($M, 0, n-1, l$) vérifie $U(l)=(n-1)(1+T(l-1))$ et par la même somme téléscopique, on obtient $U(l)=(n-1)^l=U(n^l)$. D'où $T(l)=\Omega(n^l)$.
\end{prop}
...
\begin{prop}{}{}
    ...
    \end{itemize}
    \tcblower
    On numérote les sommets selon leur ordre de marquage dans le parcours : $s_0,s_1,...,s_n$. On a l'invariant :
...
\begin{lemme}{}{}
    ...
    $\hra$ Si $u\to v$ est tendu, alors poids$(v)>$ $\nt{poids}(u)+\w(u\to v)=i+1$.\\
    ...
    $\hra$ Si $u\to v$ n'est pas tendu, alors poids$(v)\leq i$.\\
...
\begin{thm}{}{}
    À la fin de l'algorithme, poids$(v)$ est la longueur d'un plus court chemin de $s$ à $v$ dans $G$ pour tout $v$.
    \tcblower
    Soit un sommet $v$ et un chemin $s=v_0\to v_1 \to ... \to v_l = v$; $d: S \to \N, ~ v\mapsto$ la valeur finale de poids($v$).\\
    ...
    Au moment où on atteint la valeur finale de poids($v_j$), on extrait $v_j$ de la file. On regarde $v_j\to v_{j+1}$.\\
    $\hra$ S'il n'est pas tendu, poids($v_{j+1}) \leq $ poids$(v_j)+\w(v_j\to v_{j+1})=d(v_j)+1\leq j+1$\\
    Donc $d(v_{j+1})\leq$ poids($v_{j+1}) \leq j+1$.\\
    $\hra$ S'il est tendu, alors poids$(v_{j+1})$ prend la valeur poids($v_j)+\w(v_j\to v_{j+1})=d(v_j)+1$, donc poids$(v_{j+1})\leq j+1$.\\
    On applique le résultat à $j=l$ : la longueur du chemin initial est supérieure à la longueur du chemin trouvé par l'algorithme. Comme on n'a pas mis de condition sur le chemin initial, le chemin trouvé par l'algorithme est le chemin le plus court.
\end{thm}
...
\begin{defi}{Algorithme de Dijkstra. $\star$}{}
    ...
    \end{algorithm}
    En pratique, on va garder en mémoire l'indice de tous les sommets dans la file pour les \texttt{actualiser}.
\end{defi}
...
\begin{nota}{}{}
    Pour $i\in\N$, on note : $u_i$ le sommet extrait à l'itération $i$ et $d_i$ le poids associé à $u_i$ juste après l'extraction de $u_i$.
\end{nota}
