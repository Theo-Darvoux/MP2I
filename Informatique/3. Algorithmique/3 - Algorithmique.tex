...
\begin{defi}{Algorithmique.}{}
    L'\bf{algorithmique} est la branche de l'informatique qui étudie la conception et l'analyse des algorithmes.\\
    Elle comporte trois axes d'étude :\\
    $\hra$ La conception.\\
    $\hra$ La correction.\\
    $\hra$ L'efficacité.
\end{defi}
...
\begin{defi}{Problème de décision.}{}
    Quand la réponse attendue à une question est \texttt{oui} ou \texttt{non}, on parle de \bf{problème de décision}.
\end{defi}

\begin{defi}{Décidabilité.}{}
    On dit d'un problème de décision qu'il est \bf{décidable} s'il est possible de trouver un algorithme qui le résout.
\end{defi}

\subsection{Correction : critères de vérification.}

\begin{defi}{Arrêt.}{}
    On dit qu'un algorithme s'\bf{arrête} si le calcul se termine en un temps fini, quelle que soit l'entrée.
\end{defi}
...
\begin{defi}{Complexité spatiale.}{}
    La \bf{complexité spatiale} d'un algorithme est la place mémoire occupée en plus de l'entrée pendant le calcul, en fonction de la taille de l'entrée.
\end{defi}
...
\begin{defi}{Invariant.}{}
    ...
    $\hra$ Vrai avant d'entrer dans la boucle.\\
    $\hra$ S'il est vrai à l'entrée d'une itération et qu'on sort de l'itération, alors il est vrai en sortie.\\
    Si on sort de la boucle, les propriétés de l'invariant de boucle assurent qu'il est encore vrai.
\end{defi}
...
\begin{prop}{Trivial.}{}
    \begin{equation*}
        f\in\Theta(g) \iff \left[f\in O(g) \et f\in\Omega(g)\right] \iff \left[f\in O(g) \et g \in O(f)\right]
    \end{equation*}
\end{prop}
...
\begin{defi}{}{}
    Soient deux fonctions $f$ et $g$ allant de $\N$ vers $\R_+$, ne convergeant pas vers 0.
    \begin{equation*}
        \frac{f(n)}{g(n)}\xrightarrow[n\to+\infty]{}l \in \R \ra f \in O(g)
    \end{equation*}
    De plus, $f\in\Theta(g) \iff l \neq 0$.
    \tcblower
    Supposons que $f/g$ converge vers $l\in\R$. Toute suite convergente est bornée donc $\exists C\in\R_+^*, ~ f(n)<C\cdot g(n)$.\n
...
\begin{meth}{Dans le pire des cas.}{}
    ...
    $\hra$ Trouver un majorant significatif de la complexité.\\
    $\hra$ Trouver une famille d'entrées pour laquelle la complexité est minorée par $\a f(n)$, pour $\a$ constante.\\
...
\begin{defi}{}{}
    ...
    $\hra$ Comparaisons : à chaque fois.\\
    $\hra$ Affectations : pas toujours.\\
...
\begin{defi}{Algorithme de la recherche dichotomique.}{}
    \begin{algorithm}[H]
        \caption{Recherche dichotomique.}
        \Entree{Liste $L$ de taille $n$ d'éléments comparables triée dans l'ordre croissant, et un élément $x$.}
        \Sortie{Un booléen indiquant si $x\in L$.}
        milieu $\gets L[n/2]$
        \Si{milieu $ == x$}{
            \Retour{Vrai.}
        }
        \SinonSi{milieu $ > x$}{
            \Retour{\texttt{dichotomie}($L[0:n/2],~x,~n/2$)}
        }
        \Retour{\texttt{dichotomie}($L[n/2+1:n],~x,~n/2$)}
    \end{algorithm}
    où $L[i:j]$ est la liste des éléments de $L$ d'indices dans $\lb i, j \lb$. 
\end{defi}
...
\begin{prop}{}{}
    ...
    Si l'élément à trouver dans le tableau n'y est pas, on a une complexité en $\Omega(\log n)$, d'où le $\Theta(\log n)$.
\end{prop}
...
\begin{ex}{}{}
    Pour un exemple de calcul de complexité en moyenne, voir le cas du tri rapide dans le chapitre 4 - Collections de données, proposition 36.
\end{ex}

\subsection{Complexité amortie.}

\begin{defi}{}{}
    L'analyse amortie consiste à relativiser le coût élevé de certaines opérations réalisées sur une structure de données. Au lieu d'étudier les opérations de manière indépendante, on les étudie en tant que suite d'opérations dont on aimerait connaître la complexité moyenne.\\
    Cela permet notamment d'exhiber que la rapidité de certaines opérations est la conséquence de la lenteur d'autres.\\
    Par exemple, pour la structure Unir-Et-Trouver (MPI) on a des implémentations où on favorise l'union, et d'autres où on favorise la recherche du représentant.
\end{defi}

\begin{meth}{Agrégats.}{}
    On étudie une suite d'opérations, et on observe la moyenne de la complexité d'une opération.
\end{meth}

\pagebreak

\begin{ex}{Tableaux dynamiques.}{}
    On aimerait avoir une structure dans laquelle on peut insérer un élément à la fin et accéder au $i^{\nt{ème}}$ élément.\\
    Un tableau dynamique est composé de trois valeurs :\\
    $\hra$ Une taille $n\in\N$, une capacité totale $m\in\N$ et un tableau de taille $m$.\\
    Lorsqu'on ajoute un élément, on l'ajoute à la case $n$ du tableau, on incrémente $n$. Si $n$ atteint $m$, alors le tableau est plein : on en crée un nouveau de taille $\a m$ ($\a\in\R$), et on y copie les valeurs de l'ancien tableau.\\
    Si on a un tableau dynamique dans lequel on insère $k$ éléments, à chaque fois que $n$ est de la forme $\a^i$, on doit faire une insertion lente. Chaque insertion lente est moins rapide que la précédente, mais elles se font de plus en plus rares. Si on part d'un tableau dynamique vide, on a des insertions lentes pour $i\in\{j \in \N, ~ \a^j \leq k\}$.\\
    On note $c_i$ le coût de la $i^{\nt{ème}}$ opération :
    \begin{equation*}
        \frac{\sum_{i=1}^k c_i}{k} \leq \frac{\sum_{i=1}^k 1 + \sum_{j=0}^{\lf \log_\a k \rf}\a^j}{k} \leq 1 + \frac{\a^{\lf \log_\a k \rf+1}}{k} - \frac{1}{(\a-1)k} \leq 1 + \frac{\a k}{k} = O(1)
    \end{equation*}
\end{ex}

\vspace*{-0.8cm}

\begin{meth}{Potentiels.}{}
    On crée une fonction $\Phi$ et celle-ci nous renvoie une valeur selon l'état de la structure.\\
    Alors le coût amorti d'une opération est égal à son coût réel moins la variation de $\Phi$ après son exécution.
\end{meth}

\vspace*{-0.8cm}

\begin{ex}{}{}
    Pour le tableau dynamique, $\Phi=m-n$ quand on réalise une insertion rapide, $\Delta\Phi=-1$, et pour une insertion lente, $\Delta\Phi=(\a-1)n-1$. Dans les deux cas, le coût amorti est en $O(1)$.
\end{ex}

\vspace*{-0.8cm}

\begin{meth}{Banquier.}{}
    On paie un coût fictif à chaque opération, qui dépend uniquement de l'opération choisie. Les coûts en trop serviront à financer les opérations qui ont un coût réel plus élevé que leur coût fictif.
\end{meth}

\vspace*{-0.8cm}

\begin{ex}{}{}
    On a envie de fixer un coût permettant le financement des insertions lentes. L'insertion lente consiste à créer un tableau de taille $\a n$ et à recopier les $n$ premières cases. On veut donc avoir $\a n + n$ pièces dans la structure de données lors d'une insertion lente, donc que les $n$ insertions rapides précédentes aient laissé $n\a + n$ pièces.\\
    \xmark~Un coût de $\a+1$ ne fonctionne pas car lors d'une insertion lente, les $n$ valeurs réinsérées ne contribuent pas au financement.\\
    \cmark~Un coût de $\a + 1 + k$ avec $k=\frac{\a+1}{\a-1}$ fonctionne car il compense le déficit créé par ces $n$ valeurs :
    \begin{equation*}
        (\a n - n)(\a + 1 + k) \geq \a n(\a + 1) \ra k(\a - 1) \geq \a + 1 \ra k \geq \frac{\a + 1}{\a - 1}.
    \end{equation*}
    Le coût amorti d'une opération est alors le coût fictif de celle-ci.\\
    Ici, toutes les insertions ont un coût amorti en $O(\a + 1 + k) = O(1)$.
\end{ex}
