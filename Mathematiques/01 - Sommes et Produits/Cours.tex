...
\begin{corr}{La somme de la combinaison linéaire, c'est la combinaison linéaire des sommes.}{}
    ...
    \begin{equation*}
        \sum_{i\in I}(\l a_i + \mu b_i) = \l\sum_{i\in I}a_i + \mu\sum_{i\in I}b_i.
    \end{equation*}
\end{corr}
...
\begin{ex}{Calculs de sommes de progressions géométriques.}{}
...
\bf{Remarque.} Vu en Terminale: $n!$ est le nombre de permutations d'un ensemble à $n$ éléments. C'est aussi le nombre de fa\cc{c}ons différentes de numéroter $n$ objets. Sera revu dans le cours de dénombrement.
...
\begin{prop}{Produit de deux sommes.}{}
    Soient $n$ et $p$ dans $\N^*$ et deux familles de nombres complexes $(a_i)_{i\in\lb1,n\rb}$ et $(b_i)_{i\in\lb1,p\rb}$. On a
    \begin{equation*}
...
\begin{exercice}{$\blacklozenge\blacklozenge\lozenge$}{}
    ...
    1. À l'aide d'un téléscopage, démontrer l'identité : $\sum\limits^{n}_{k=p}{\binom{k}{p}}=\binom{n+1}{p+1}$.\\
...
\begin{exercice}{$\blacklozenge\blacklozenge\lozenge$}{}
    ...
    On a :
    \begin{align*}
        \sum\limits_{0\leq{i}\leq{j}\leq{n}}{2^{-j}\binom{j}{i}}=\sum\limits^{n}_{j=0}{2^{-j}\sum\limits^{j}_{i=0}{\binom{j}{i}}}=\sum\limits^{n}_{j=0}{2^{-j}\cdot2^j}=n+1
    \end{align*}
\end{exercice}
...
\begin{exercice}{$\blacklozenge\blacklozenge\lozenge$}{}
    ...
    On a :
    \begin{align*}
        \sum\limits_{1\leq i,j\leq n}{|i-j|}&=\sum\limits^{n}_{j=1}{\sum\limits^{n}_{i=1}{|i-j|}}=\sum\limits^{n}_{j=1}{\left(\sum\limits^{j}_{i=1}{|i-j|}+\sum\limits^{n}_{i=j+1}{|i-j|}\right)}\\
        &=\sum\limits^{n}_{j=1}{\left(\sum\limits^{n}_{i=j+1}{(i-j)}-\sum\limits^{j}_{i=1}{(i-j)}\right)}=\sum\limits^{n}_{j=1}{\sum\limits^{n-j}_{i=1}{i}-\sum\limits^{n}_{j=1}\sum\limits^{j}_{i=1}{i}+\sum\limits^n_{j=1}j^2}\\
    \end{align*}
    Or,
    \begin{equation*}
