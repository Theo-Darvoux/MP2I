...
\begin{defi}{}{}
    On appelle \bf{polynôme} à coefficients dans $\K$ une suite d'éléments de $\K$ nulle àpdcr\n
...
\begin{corr}{}{}
    Soit $P=(a_k)_{k\in\N}$ un polynôme de $\K[X]$ et $m$ un entier tel que $\forall k > m, ~ a_k = 0$. Alors
    \begin{equation*}
        P=\sum_{k=0}^ma_kX^k
    \end{equation*}
\end{corr}
...
\begin{ex}{Formule de Vandermonde. $\star$}{}
    ...
    \end{equation*}
    \tcblower
    On a $(X+1)^p(X+1)^q=(X+1)^{p+q}=\sum_{k=0}^{p+q}\binom{p+q}{k}X^k$ d'une part. D'autre part :
    \begin{equation*}
...
\begin{enumerate}
    ...
    \item De la même fa\cc{c}on, on écrira $P(X^2)$, ou $P(Q(X))$ pour désigne $P\circ X^2$ et $P\circ Q$.
...
\begin{corr}{}{}
    ...
    \end{equation*}
    Ainsi, nous pouvons << simplifier >> par un polynôme non nul :
    \begin{equation*}
...
\begin{defi}{}{}
    Soit $P\in\K[X]$ et $k\in\N$. On définit la \bf{dérivée $k$-ème} de $P$, que l'on note $P^{(k)}$, en posant
    \begin{equation*}
...
\begin{corr}{}{}
    Soit $(A,B)\in\K[X]^2$, avec $B\neq0$.\\
    Alors $B$ divise $A$ ssi le reste dans la division euclidienne de $A$ par $B$ est le polynôme nul.
    \tcblower
    \boxed{\la} Trivial.\\
...
\begin{ex}{}{}
    ...
    Donc $j,\ov{j}$ sont racines distinctes de $P$, donc $(X-j)(X-\ov{j})\mid P$ dans $\C[X]$ : $\exists Q \in \C[X] \mid P=(X^2+X+1)Q$.
\end{ex}
...
\begin{corr}{}{}
    ...
    \end{equation*}
    \tcblower
    On a $\exists Q \in \K[X] \mid P = Q\prod_{i=1}^n(X-\a_i)$ donc $\deg(P)=\deg(Q)+n$ donc $\deg(Q)=0$ donc $\exists \l \in \K^* \mid Q=\l\cdot 1_{\K[X]}$.
\end{corr}
...
\begin{ex}{Factorisation des polynômes de Tchebychev.}{}
    ...
    Donc les nombres $\cos\theta_k$ avec $k\in\lb0,n-1\rb$ sont distincts par stricte décroissance de $\cos$ sur $[0,\pi]$.\\
...
\begin{ex}{$\star$}{}
    ...
    \tcblower
    On a $X^6-1 = (X+1)(X-1)(X-j)(X-j^2)(X+j)(X+j^2) = (X-1)(X+1)(X^2-X+1)(X^2+X+1)$
\end{ex}
...
\begin{ex}{}{}
    ...
    \begin{equation*}
        x^2+y^2+z^2 = \s_1^2-2\s_2; \quad
        x^3+y^3+z^3=\s_1^3+3\s_3-3\s_1\s_2
    \end{equation*}
    \tcblower
    On a $\s_1^2-2\s_2=x^2+\cancel{xy+xz+yx}+y^2+\cancel{yz+zx+zy}+z^2-\cancel{2xy-2xz-2yz}=x^2+y^2+z^2$.
\end{ex}
...
\begin{prop}{Relations coefficients-racines : formules de Viète. $\star$}{}
    ...
    Ces nombres s'expriment donc en fonction des coefficients de $P$ : $\forall k \in \lb1,n\rb, ~ \s_k = (-1)^k\frac{a_{n-k}}{a_n}$.\\
...
\begin{corr}{L'ensemble des polynômes interpolateurs.}{}
    ...
    \end{equation*}
    \tcblower
    On a $\forall i \in \lb 1,n \rb, ~ Q(x_i)=y_i=P(x_i) \iff Q-P \nt{ a } x_1,...,x_n \nt{ pour racines} \iff \prod_{i=1}^n(X-x_i)\mid Q-P$.\\
    Puis $\prod_{i=1}^n(X-x_i) \mid Q-P \iff \exists A \in \K[X] , ~ Q=P+A\prod_{i=1}^n(X-x_i)$.
\end{corr}
...
\begin{exercice}{$\bbw$ $\star$}{}
    ...
    \begin{enumerate}
        \item Montrer que pour tout $n\in\N$, il existe un polynôme $P_n\in\R[X]$ tel que
        \begin{equation*}
...
\begin{exercice}{$\bww$}{}
    En calculant de deux fa\cc{c}ons différentes le coefficient devant $X^n$ dans l'écriture de $(1-X^2)^n$, obtenir une identité sur les coefficients binomiaux.
    \tcblower
    D'une part
    \begin{align*}
...
\begin{exercice}{$\bbw$}{}
    ...
    \end{equation*}
    On a $\ds\forall k \in \lb 1, n\rb, ~ |\s_k|=\left|\sum_{I\in\P_k(\lb1,n\rb)}\prod_{i\in I}\w_i\right|\leq\sum_{I\in\P_k(\lb1,n\rb)}\prod_{i\in I}1=\binom{n}{k}$.\\
...
\begin{exercice}{$\bbb$}{}
    Soit $n\in\N$.
    \begin{enumerate}
        \item Exprimer de deux fa\cc{c}ons différentes l'unique polynôme $P$ de degré $n$ tel que $\forall i \in \lb0,n\rb, ~ P(i)=i^n$.
